...
const GoogleSVG = () => (
  <svg className="google-icon" viewBox="0 0 18 18" fill="none">
    ...
    <path fill="#EA4335"
      d="M9 3.6c1.3 0 2.5.4 3.4 1.3L15 2.3A9 9 0 001 5l3 2.4a5.4 5.4 0 015-3.8z" />
  </svg>
);
...
/* ─── Inline signup validator (runs on every keystroke) ── */
function validateSignupInline(name, value, allData) {
  // Don't show "required" errors while the field is still empty and untouched
  // But DO show format errors the moment something wrong is typed

  if (name === 'name') {
    if (!value) return '';                                          // empty → silent
  ...
  if (name === 'email') {
    ...
    if (!emailRegex.test(value.trim())) return 'Enter a valid email address (e.g. you@example.com)';
  }
  ...
  if (name === 'phone') {
    ...
    if (value.length > 0 && value.length < 10)
      return `${10 - value.length} more digit${10 - value.length > 1 ? 's' : ''} needed`;
  }
...
/* ─── Inline login validator ── */
function validateLoginInline(name, value) {
  if (name === 'identifier') {
    if (!value) return '';
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    const phoneRegex = /^[6-9]\d{9}$/;
    // Only show error once they've typed something meaningful
...
export default function Auth() {
  ...
  /* ── login field change — validate every keystroke ── */
  ...
  /* ── login blur — run full validation to catch empty fields ── */
  ...
  /* ── signup field change — validate every keystroke ── */
  function onSignupChange(e) {
    const { name, value } = e.target;

    if (name === 'phone') {
      // Strip non-digits immediately (no letters ever get stored)
    ...
    // If password changes, also re-check confirmPassword live
  ...
  /* ── signup blur — run full validation to catch empty fields ── */
  ...
  function handleLogin(e) {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem('userName', loginData.identifier.split('@')[0]);
      setLoading(false);
      navigate('/dashboard');
    }, 1000);
  }

  /* ── signup submit — on success go back to Sign In tab ── */
  ...
  function handleGoogle() {
    alert('Google Sign-In integration — connect your OAuth provider here.');
  }
