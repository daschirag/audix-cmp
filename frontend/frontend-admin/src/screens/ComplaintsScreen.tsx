...
import { complaintsData, Complaint } from '../data/mockData'
...
const owners = ['Priya M.', 'Amit K.', 'Rajesh S.', 'Sneha K.']
...
export default function ComplaintsScreen() {
  ...
  const handleStatusUpdate = () => {
    ...
    setComplaints((prev) =>
      prev.map((c) =>
        selected.has(c.id) ? { ...c, status: selectedStatus as Complaint['status'] } : c
      )
    )
    setSelected(new Set())
    setActiveModal(null)
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {filtered.map((complaint) => {
                const sc = statusColors[complaint.status]
                const slac = slaColors[complaint.sla]
                return (
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl">
            ...
            {activeModal === 'remarks' && (
              <>
                <textarea
                  ...
                  rows={4}
                  placeholder="Enter internal notes or remarks..."
            ...
            {activeModal === 'evidence' && (
              <>
                {!uploadSuccess ? (
                  <div
                    className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center cursor-pointer hover:border-[#00C4B4] transition-colors mb-4"
