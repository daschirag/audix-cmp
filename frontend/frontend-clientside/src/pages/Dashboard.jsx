...
const PURPOSE_NAMES = {
  ...
  'purpose-analytics':  'Analytics and Improvement',
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Fetch real consents
    fetch(`${API}/consent/status/${u.id}`, {
      headers: { Authorization: `Bearer ${token()}` }
    })
      .then(r => r.json())
      .then(data => {
        if (data.success && Array.isArray(data.data)) {
          setConsents(data.data.filter(c => c.status === 'ACTIVE'))
        }
      })
      .catch(() => {})

    // Fetch real complaints for this user
      ...
      .then(data => {
        if (data.success && Array.isArray(data.data)) {
          ...
          setComplaints(mine.map(c => ({
            ...
            status:      c.status === 'RECEIVED' ? 'Submitted'
  ...
  const activeConsentsCount = consents.length
  ...
  function onProfileChange(e) {
    ...
    if (profileTouched[name]) {
      setProfileErr(p => ({ ...p, [name]: validateProfileField(name, cleaned) }))
    }
  }
  ...
  // Withdraw consent
  function toggleItem(consentId) {
    setSelectedIds(prev =>
      prev.includes(consentId) ? prev.filter(id => id !== consentId) : [...prev, consentId]
    )
  }
  ...
  async function submitComplaint(e) {
    ...
    try {
      const res = await fetch(`${API}/complaints`, {
        ...
        body:    JSON.stringify({
          dataPrincipalId: u?.id,
          consentId:       consents[0]?.id || u?.id,
  ...
  function exportData() {
    const blob = new Blob(
      [JSON.stringify({ consents, complaints, exportDate: new Date().toISOString() }, null, 2)],
    ...
    <div className="dashboard-root">
          ...
          <div className="header-right">
            <div className="header-user-info">
              ...
              <div className="consent-date">
                {consents.length > 0 ? `Last consent: ${lastUpdated}` : 'No consent data'}
              </div>
            </div>
        ...
        <aside className="sidebar">
          ...
          <div className="sidebar-stats">
            <div className="sidebar-stats-top">
              ...
              <div>
                <div className="stats-count">{activeConsentsCount}</div>
            ...
            <div className="card">
              ...
              {loading ? (
                    ...
                    <div className="consent-item" key={c.id}>
                      <div className="consent-item-header">
                        ...
                        <span className="badge badge-blue">
                          {c.purposes?.length || 0} purpose{(c.purposes?.length || 0) !== 1 ? 's' : ''}
                        </span>
                      </div>
                      <div className="consent-tags">
                        {(c.purposes || []).map((p, j) => (
                          <span key={j} className="consent-tag consent-tag-optional">
                            {p.purposeId}
                          </span>
                        ))}
                      </div>
                      <div style={{ fontSize: 12, color: '#888', marginTop: 4 }}>
                        Granted: {new Date(c.createdAt).toLocaleDateString('en-IN')}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="empty-state">
                  ...
                  <a href="/consent-form">Go to consent form â†’</a>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="card">
              ...
              {loading ? (
                ...
                <div className="withdraw-items">
                  {consents.map((c) => (
                    <label className="withdraw-row" key={c.id}>
                      <div className="withdraw-row-left">
                        <input type="checkbox" className="withdraw-checkbox"
                          checked={selectedIds.includes(c.id)}
                          onChange={() => toggleItem(c.id)} />
                        <div>
                          <div className="withdraw-field-name">
                            Consent ID: {c.id.slice(-8).toUpperCase()}
                          </div>
                          <div className="withdraw-category">
                            {(c.purposes || []).map(p => purposeName(p.purposeId)).join(', ')}
                          </div>
                        </div>
                      </div>
                      <span className="badge badge-slate">Can Withdraw</span>
                    </label>
                  ))}
                </div>
              ) : (
              ...
              <div className="withdraw-actions">
                <button className="btn-danger" onClick={processWithdrawals}>
                  <Icon d={ICONS.cross} size={18} /> Confirm Withdrawal
                </button>
              </div>
            </div>
          </div>
                ...
                <div className="form-grid-2">
                  <div className="form-group">
                    ...
                    <select name="category"
                      className={`form-control ${complaintErr.category ? 'form-control-error' : ''}`}
                    ...
                    {complaintErr.category && <span className="form-field-error">âš  {complaintErr.category}</span>}
                  </div>
                  <div className="form-group">
                    <label>Priority Level <span>*</span></label>
                    <select name="priority"
                      className={`form-control ${complaintErr.priority ? 'form-control-error' : ''}`}
                    ...
                    {complaintErr.priority && <span className="form-field-error">âš  {complaintErr.priority}</span>}
                  </div>
                </div>
                <div className="form-group">
                  <label>Subject <span>*</span></label>
                  <input type="text" name="subject"
                    className={`form-control ${complaintErr.subject ? 'form-control-error' : ''}`}
                    placeholder="Brief description of your complaint"
                    value={complaint.subject} onChange={onComplaintChange} onBlur={onComplaintBlur} />
                  {complaintErr.subject && <span className="form-field-error">âš  {complaintErr.subject}</span>}
                </div>
                <div className="form-group">
                  <label>Detailed Description <span>*</span></label>
                  <textarea name="description"
                    className={`form-control ${complaintErr.description ? 'form-control-error' : ''}`}
                    placeholder="Please provide detailed information..."
                    value={complaint.description} onChange={onComplaintChange} onBlur={onComplaintBlur} />
                  {complaintErr.description && <span className="form-field-error">âš  {complaintErr.description}</span>}
                </div>
                <div className="form-group">
                  <label>Contact Email <span>*</span></label>
                  <input type="email" name="email"
                    className={`form-control ${complaintErr.email ? 'form-control-error' : ''}`}
                    placeholder="your.email@example.com"
                    value={complaint.email} onChange={onComplaintChange} onBlur={onComplaintBlur} />
                  {complaintErr.email && <span className="form-field-error">âš  {complaintErr.email}</span>}
                </div>
                <div className="complaint-info-box">
                  <Icon d={ICONS.warn} size={18} />
                  <div>
                    <div className="info-title">Response Timeline</div>
                    <div className="info-text">We will acknowledge your complaint within 24 hours and resolve within 7 business days as per DPDPA guidelines.</div>
                  </div>
                </div>
                <div className="form-actions">
                  <button type="submit" className="btn-primary">
                    <Icon d={ICONS.send} size={16} /> Submit Complaint
                  </button>
                    ...
                    return (
                      <div className="complaint-card" key={c.id}>
                        ...
                        <div className="complaint-desc">
                          {c.description?.length > 120 ? c.description.substring(0, 120) + 'â€¦' : c.description}
                        </div>
                      </div>
                    )
                  })
          ...
          <div className={`tab-panel ${activeTab === 'settings' ? 'active' : ''}`}>
            <div className="card">
              ...
              <div className="settings-section">
                ...
                {[
                  { label: 'Consent Update Notifications', sub: 'Receive alerts when consents are modified', defaultOn: true },
              ...
              <div className="settings-section">
                ...
                <div className="export-box">
                  ...
                  <div>
                    ...
                    <p>Download a complete copy of your consent records in JSON format.</p>
      ...
      <Modal open={profileOpen} onClose={() => { setProfileOpen(false); cancelEdit() }}>
        <div className="modal-box profile-modal-box">
          ...
          {profileSaved && <div className="profile-success-banner">âœ… Profile updated successfully</div>}
          <div className="profile-fields">
            {[
              ...
              <div className="profile-field-group" key={field.key}>
                <label className="profile-label">
                  <Icon d={field.icon} size={14} /> {field.label}
                </label>
                {isEditing ? (
                  <>
                    ...
                    {profileErr[field.key] && <span className="profile-field-error">âš  {profileErr[field.key]}</span>}
                  </>
                ) : (
            ...
            <div className="profile-dpdpa-badge">
              <Icon d={ICONS.shield} size={14} /> Your data is protected under DPDPA 2023
            </div>
          </div>
          <div className="profile-actions">
            {isEditing ? (
              <>
                <button className="btn-primary" onClick={saveProfile}><Icon d={ICONS.check} size={16} /> Save Changes</button>
                <button className="btn-reset" onClick={cancelEdit}>Cancel</button>
              </>
            ) : (
              <>
                <button className="btn-primary" onClick={() => setIsEditing(true)}><Icon d={ICONS.edit} size={16} /> Edit Profile</button>
